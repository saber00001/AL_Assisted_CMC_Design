...
input0 = input0.drop(input0.columns[4], axis=1)
input0_np = input0.values[1:, 1:]
output0_np = output0.values[1:, 1:]

print("input0_np: ")
print(input0_np.shape)

print("output0_np: ")
print(output0_np.shape)
...
def optimize_qnehvi_and_get_observation(model, train_x, train_obj, sampler, loop):
    ...
    (torch.tensor([6]), torch.tensor([input_scale[6]]).to(**tkwargs), -(standard_input_bounds[0, 6] - 2.2))
    ]
    ...
    target = 2.0 * nor_means1[:, 1] + 1.0 * nor_means1[:, 0] + 1.0 * nor_means1[:, 2]
    ...
    input_column = ['CMC/g', 'Corn starch/g',	'Tapioca/g',	'H2O/mL',	'10% wt urea solution/mL',
                 	'10% wt NaOH solution/mL', 'urea/NaOH', '10% wt ammonia solution/mL',	'Glycerol/mL'	, 'Temperature/℃'
    ]
    
    ratio = new_x_np[:, 3]/new_x_np[:, 4]
    vol_H2O = 50 - (new_x_np[:, 3] + new_x_np[:, 4] + new_x_np[:, 5] + new_x_np[:, 6])
    new_x_np_insert = np.insert(new_x_np, 3, vol_H2O, axis=1)
    new_x_np_insert = np.insert(new_x_np_insert, 6, ratio, axis=1)
...
for iteration in range(1, N_BATCH + 1):
    ...
    optimize_qnehvi_and_get_observation(
        model_qnehvi, x_qparego, obj_qparego, qnehvi_sampler,
        iteration
    )
